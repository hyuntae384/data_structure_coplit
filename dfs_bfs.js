/* 
아래와 같이 해도 결과값은 제대로 나오나 
DFS와 BFS를 사용한 것이 아니기에 테스트는 통과되지 않는다
*/
function connectedVertices(edges) {
  ...
  for(let i=0; i<edges.length; i++){
    if(!start.includes(edges[i][0])){
      start.push(edges[i][0])
    }
  }
  for(let j=0; j<edges.length; j++){
    if(start.includes(edges[j][1])){
      comeback = comeback + 1
    }
  }

  const result = start.length - comeback
